...
class TestIntentDetectionAgent:
    ...
    async def test_detect_troubleshooting_intent(self, agent: IntentDetectionAgent) -> None:
        ...
        mock_response.content = '{"intent": "troubleshooting", "confidence": 0.95, "entities": {"affected_service": "api-gateway"}, "reasoning": "User reports errors"}'
        with patch.object(agent.router, "chat", new_callable=AsyncMock, return_value=mock_response):
    ...
    async def test_detect_low_risk_access_intent(self, agent: IntentDetectionAgent) -> None:
        mock_response = MagicMock()
        mock_response.content = '{"intent": "low_risk_access", "confidence": 0.9, "entities": {"resource_type": "jira", "access_level": "read"}, "reasoning": "Read access request"}'
        with patch.object(agent.router, "chat", new_callable=AsyncMock, return_value=mock_response):
    ...
    async def test_detect_high_risk_access_intent(self, agent: IntentDetectionAgent) -> None:
        mock_response = MagicMock()
        mock_response.content = '{"intent": "high_risk_access", "confidence": 0.92, "entities": {"resource_type": "production_database", "access_level": "admin"}, "reasoning": "Production database admin access"}'
        with patch.object(agent.router, "chat", new_callable=AsyncMock, return_value=mock_response):
    ...
    async def test_detect_handles_llm_failure(self, agent: IntentDetectionAgent) -> None:
        with patch.object(agent.router, "chat", new_callable=AsyncMock, side_effect=Exception("API error")):
